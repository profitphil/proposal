# Smart Contract - Qubic Payment Hub (QPAY)

## Proposal
Allow Deployment of the QPAYHUB Smart Contract on Qubic
...
QPAYHUB is the settlement layer for x402 pay-per-request commerce on Qubic — the standard where a server answers with `HTTP 402 Payment Required`, takes payment, then serves the resource. Feeless transfers that finalize in seconds make charging per API call practical on Qubic; QPAYHUB adds the missing piece: a verifiable on-chain receipt that a specific buyer paid a specific seller for a specific resource, checkable before delivery and impossible to spend twice.

- Settles **non-custodially** — the seller's net arrives in the same transaction
- Records a **single-use receipt** per purchase, readable by anyone
- Retains **0.75%** (min 100 QU), distributed each epoch to QPAY holders and shareholders
- Publishes a **live QUBIC/USD price** from Qubic's Oracle Machines
- **No custody, no seizure power** for any identity, privileged or not
- **Permissionless** — any identity can sell, with no registration
...
|---|---|---|---|
| 1 | `GetReceipt` | 1 | `Pay` |
| 2 | `ComputeReceiptKey` | 2 | `Consume` |
| 3 | `GetInfo` | 3 | `SubscribeToPriceFeed` |
| 4 | `GetQuUsdPrice` | 4 | `SetPromoRate` |
| 5 | `GetPromoRate` | 5 | `RemovePromoRate` |
| 6 | `GetAffiliate` | 6 | `ChangeOperator` |
| | | 7 | `SetAffiliate` |
| | | 8 | `RemoveAffiliate` |
| | | 9 | `ChangeAffiliateRegistrar` |

Plus `NotifyQuUsdPriceReply`, the oracle callback.

A buyer invokes `Pay` with `{seller, resourceId, nonce}` and the price attached. The contract retains the fee, pushes the net to the seller in the same transaction, and records the receipt. The seller reads `GetReceipt` to confirm payment before serving, and may call `Consume` to mark it spent on-chain.

---

## Fees

- **0.75%, or 100 QU, whichever is greater**, clamped so the fee can never exceed the payment. The floor dominates below ~13,333 QU.
- Minimum payment 100 QU; anything lower is rejected and refunded.
- **Promo rates** — an operator key may discount a seller within a contract-enforced 0.25%–0.75% band. Maximum 32 sellers.
- **Affiliates** — a registrar key may attribute a seller to an affiliate earning 5% of the *fee*, never the seller's net. Maximum 128.
- Fees are **not** returned when a seller refunds a buyer, as with card processors.

## Receipts

Keyed `K12(payer, seller, resourceId, nonce)`, capacity 262,144, expiring after 2 epochs and purged at epoch end. Duplicates within that window are rejected; every error path refunds the full invocation reward.

---

## Revenue Distribution

Fees accrue to an on-chain pool. Above a permanent 1,000,000 QU execution reserve, each epoch distributes:

- **89%** → QPAY token holders, pro-rata
- **10%** → QPAYHUB shareholders
- **1%** → the contract's own execution funding via `qpi.burn()`, taken from the token side so the shareholder slice is untouched

Holdings are evaluated at distribution; the issuer's and the contract's own holdings are excluded from both the payout and the denominator. If no eligible holder exists, the token portion stays in the pool for a later epoch.

---

## Price Oracle

QPAYHUB subscribes to Qubic's Oracle Machines for a QUBIC/USD price and republishes it via `GetQuUsdPrice`. `SubscribeToPriceFeed` is permissionless and takes no caller-supplied parameters — the oracle and pair are fixed constants, so anyone may renew a lapsed subscription without redirecting the feed. Stale prices are reported as stale.

---

## Security Model

No procedure can move funds belonging to another identity, and the contract never holds a seller's proceeds. Three narrow keys exist:

| Key | Can do | Cannot do |
|---|---|---|
| **Operator** | Set/remove a promo rate within 0.25%–0.75% | Move funds, raise fees, alter receipts |
| **Registrar** | Attribute/clear an affiliate | Move funds, alter rates or receipts |
| **Recovery** | Rotate the two keys above | Everything else |

A compromised operator key can only make the protocol charge *itself* less; a compromised registrar key can only misdirect part of the protocol's own fee. Neither reduces a seller's proceeds or reaches any balance. Rates, splits, and retention are compile-time constants — there is no pause, upgrade, or migration path.

Excluded by design: no delivery escrow (no oracle can attest an HTTP response arrived) and no on-chain pricing (the 402 challenge stays off-chain).

---

## Testing

11/11 GoogleTest unit tests and 19/19 live-chain integration checks pass, covering fee math across rounding boundaries, refunds on every error path, replay rejection and post-expiry reuse, seller-only `Consume`, epoch purging, and dividend distribution above the reserve. Full suite in `test/contract_qpayhub.cpp`.

---

## Conclusion

Deploying QPAYHUB gives Qubic:

- Non-custodial settlement with verifiable, single-use payment receipts
- Permissionless selling — no registry, no gatekeeper
- Protocol revenue to QPAY holders and shareholders every epoch
- A public QUBIC/USD price feed from Qubic's own Oracle Machines
- Privileged keys too narrow to reduce a seller's proceeds or reach a balance

This serves a category legacy rails cannot: payments worth fractions of a cent, settled in seconds, without an account.

## Technical implementation
From: https://github.com/qubic/core/pull/977
